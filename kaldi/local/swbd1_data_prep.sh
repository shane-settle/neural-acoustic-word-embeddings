...
awk '{ 
       ...
       printf("%s-%s_%06.0f-%06.0f", 
              name, side, int(100*stime+0.5), int(100*etime+0.5));
       for(i=4;i<=NF;i++) printf(" %s", $i); printf "\n"
}' $dir/swb_ms98_transcriptions/*/*/*-word.text | sort > $dir/words1.txt

sort -c $dir/words1.txt || exit 1; # check it's sorted.
...
cat $dir/words1.txt \
  | perl -ane 's:\s\[SILENCE\](\s|$):$1:gi; 
               ...
               s/<E_ASIDE>//gi; 
               print;' \
  | awk '{if(NF > 1) { print; } } ' > $dir/words2.txt

local/swbd1_map_words.pl -f 2- $dir/words2.txt > $dir/words
# --------------------------------------

# -- Note: removed from original script
...
# script]
mkdir -p data/train
for f in spk2utt utt2spk wav.scp text segments reco2file_and_channel words; do
